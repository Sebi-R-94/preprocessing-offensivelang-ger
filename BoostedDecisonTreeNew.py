...
def run(args):
    ...
    clf = GridSearchCV(classifier, params, scoring="f1_macro", verbose = 1)
    ...
    print("--- %s seconds ---" % (time.time() - start_time))
    clf = AdaBoostClassifier(DecisionTreeClassifier(criterion = "entropy", splitter="random", max_depth=300), best_p["n_estimators"], best_p["learning_rate"])
    clf.fit(X_train, y_train)
    ...
    name_resultfile = "result_adaboost_updated" + args.test[22:]
